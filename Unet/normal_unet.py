...
def save_checkpoint(model, optimizer, epoch, metrics, filepath='normal/checkpoints'):
    """
    保存模型检查点
...
def save_prediction_images(images, masks, preds, epoch, batch_idx, save_dir='normal-predictions'):
    """
    保存预测图像和真实图像的对比
...
class FirstFloodNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=12, c_list=[16,32,48,64,96,128], 
        ...
        # 跳跃连接的卷积
        self.skip_conv1 = nn.Conv2d(c_list[4], c_list[4], 1)

        self.decoder2 = nn.Sequential(
            nn.Conv2d(c_list[4] * 2, c_list[3], 3, stride=1, padding=1),
            nn.BatchNorm2d(c_list[3]),
            nn.GELU()
        )
        self.skip_conv2 = nn.Conv2d(c_list[3], c_list[3], 1)

        self.decoder3 = nn.Sequential(
            nn.Conv2d(c_list[3] * 2, c_list[2], 3, stride=1, padding=1),
            nn.BatchNorm2d(c_list[2]),
            nn.GELU()
        )
        self.skip_conv3 = nn.Conv2d(c_list[2], c_list[2], 1)

        self.decoder4 = nn.Sequential(
            nn.Conv2d(c_list[2] * 2, c_list[1], 3, stride=1, padding=1),
            nn.BatchNorm2d(c_list[1]),
            nn.GELU()
        )
        self.skip_conv4 = nn.Conv2d(c_list[1], c_list[1], 1)

        self.decoder5 = nn.Sequential(
            nn.Conv2d(c_list[1] * 2, c_list[0], 3, stride=1, padding=1),
            nn.BatchNorm2d(c_list[0]),
            nn.GELU()
        )
        self.skip_conv5 = nn.Conv2d(c_list[0], c_list[0], 1)
    ...
    def forward(self, x):
        ...
        skip5 = self.skip_conv1(x5)
        out5 = torch.cat([out5, skip5], dim=1)

        out4 = self.decoder2(out5)
        out4 = F.interpolate(out4, size=x4.shape[2:], mode='bilinear', align_corners=True)
        skip4 = self.skip_conv2(x4)
        out4 = torch.cat([out4, skip4], dim=1)

        out3 = self.decoder3(out4)
        out3 = F.interpolate(out3, size=x3.shape[2:], mode='bilinear', align_corners=True)
        skip3 = self.skip_conv3(x3)
        out3 = torch.cat([out3, skip3], dim=1)

        out2 = self.decoder4(out3)
        out2 = F.interpolate(out2, size=x2.shape[2:], mode='bilinear', align_corners=True)
        skip2 = self.skip_conv4(x2)
        out2 = torch.cat([out2, skip2], dim=1)

        out1 = self.decoder5(out2)
        out1 = F.interpolate(out1, size=x1.shape[2:], mode='bilinear', align_corners=True)
        skip1 = self.skip_conv5(x1)
        out1 = torch.cat([out1, skip1], dim=1)
        c_list=[16,32,48,64,96,128]
        # 深度监督
        if self.gt_ds:
            out5_gt = F.interpolate(self.gt_conv1(out5[:, :c_list[4]]), scale_factor=16, mode='bilinear', align_corners=True)
            out4_gt = F.interpolate(self.gt_conv2(out4[:, :c_list[3]]), scale_factor=8, mode='bilinear', align_corners=True)
            out3_gt = F.interpolate(self.gt_conv3(out3[:, :c_list[2]]), scale_factor=4, mode='bilinear', align_corners=True)
            out2_gt = F.interpolate(self.gt_conv4(out2[:, :c_list[1]]), scale_factor=2, mode='bilinear', align_corners=True)
            out1_gt = F.interpolate(self.gt_conv5(out1[:, :c_list[0]]), scale_factor=1, mode='bilinear', align_corners=True)

            out0 = F.interpolate(self.final(out1[:, :c_list[0]]), scale_factor=1, mode='bilinear', align_corners=True)
            
            return (
                torch.sigmoid(out5_gt), 
                torch.sigmoid(out4_gt), 
                torch.sigmoid(out3_gt), 
                torch.sigmoid(out2_gt), 
                torch.sigmoid(out1_gt)
            ), torch.sigmoid(out0)
        else:
            out0 = F.interpolate(self.final(out1[:, :c_list[0]]), scale_factor=1, mode='bilinear', align_corners=True)
...
def main():
    ...
    # 模型
    model = FirstFloodNet(num_classes=1, input_channels=12).to(device)
