...
def save_checkpoint(model, optimizer, epoch, metrics, filepath='checkpoints'):
    """
    保存模型检查点
...
def save_prediction_images(images, masks, preds, epoch, batch_idx, save_dir='improved-predictions'):
    """
    保存预测图像和真实图像的对比
    ...
    random_indices = np.random.choice(images.size(0), num_samples, replace=False)
...
class LightTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_flat = x_pooled.flatten(2).permute(2, 0, 1)
        x_flat = self.norm(x_flat)
        
        attn_output, _ = self.attn(x_flat, x_flat, x_flat)
        
        # 上采样
        attn_output = attn_output.permute(1, 2, 0).view(B, C, H//4, W//4)
...
class ThirdFloodNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=12, c_list=[16,32,48,64,96,128], 
        ...
        # 新增的3x3卷积和Transformer处理
        # 新增的Transformer处理模块
        self.transformer_conv = nn.Sequential(
            nn.Conv2d(c_list[0], 16, kernel_size=3, padding=1),
            nn.BatchNorm2d(16),
            nn.GELU()
        )
        self.transformer_block = LightTransformerBlock(dim=16)
        self.transformer_fusion = nn.Sequential(
            nn.Conv2d(c_list[0]+16, c_list[0], kernel_size=3, padding=1),
            nn.BatchNorm2d(c_list[0]),
            nn.GELU()
        )
    ...
    def forward(self, x):
        ...
        # 对gt_pre1进行Transformer处理
        # out1 = self.final(out1)
        
        # 对out1进行Transformer处理并融合
        trans_input = self.transformer_conv(out1)
        trans_output = self.transformer_block(trans_input)
        
        # 跳跃连接融合
        trans_fused = torch.cat([out1, trans_output], dim=1)
        final_output = self.transformer_fusion(trans_fused)
        final_output = self.final(final_output)
        ...
        if self.gt_ds:
            ...
            return (gt_pre5, gt_pre4, gt_pre3, gt_pre2, gt_pre1), final_output
        else:
            return final_output
# Custom Dataset
...
class CrossEntropyLoss(nn.Module):
    def __init__(self, weight=None, reduction='mean'):
        ...
        Args:
            weight (torch.Tensor, optional): 每个类别的权重
            reduction (str, optional): 损失reduction方法 ('mean' or 'sum')
        """
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        Args:
            pred (torch.Tensor): 模型预测输出 (B, 1, H, W)
        ...
        """
        # 确保pred和target形状一致
        pred = pred.squeeze(1)  # 移除单一通道维度
        target = target.squeeze(1)
        
        # 对于二值分割，使用BCEWithLogitsLoss
        # 这个Loss结合了Sigmoid激活和二值交叉熵
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # 使用sigmoid激活
        bcelog = F.binary_cross_entropy_with_logits(
            pred, target, reduction='none'
        )
        
        # Focal Loss计算
        pt = torch.exp(-bcelog)
        focal_loss = self.alpha * (1 - pt)**self.gamma * bcelog
